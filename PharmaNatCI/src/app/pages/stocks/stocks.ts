...
export class StocksComposant {
  ...
  inventaire = computed(() => {
    ...
    return this.serviceDonnees.stocksParEtablissementView()
      ...
      .map(med => {
        ...
        // Si CMM > 0, couverture = (stock / CMM) * 30 jours
        // Si CMM = 0 et stock > 0, on considère une couverture "infinie" (999 jours pour l'affichage)
        const joursCouverture = med.cmm > 0 ? Math.round((med.stock / med.cmm) * 30) : (med.stock > 0 ? 999 : 0);
        
        let statut = 'Normal';
        if (med.stock === 0) {
          statut = 'Rupture';
        } else if (med.stock <= med.seuilAlerte || joursCouverture < 30) {
          statut = 'Stock Bas';
        } else if (joursCouverture > 180) { // Plus de 6 mois
          statut = 'Surstock';
        }
  ...
  inventaireFiltre = computed(() => {
    return this.inventaire().filter(item => {
      ...
      const correspondLot = this.lotFiltre() === '' || (item.lotNumeros && item.lotNumeros.some((l: string) => l.toLowerCase().includes(this.lotFiltre().toLowerCase())));
      const correspondFournisseur = this.fournisseurFiltre() === '' || (item.fournisseurIds && item.fournisseurIds.includes(this.fournisseurFiltre()));
      
      return correspondRecherche && correspondCategorie && correspondStatut && correspondLot && correspondFournisseur;
