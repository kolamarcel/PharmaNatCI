...
export class Mouvements {
  ...
  fournisseurSelectionneId = signal<string>('');
  ...
  mouvementsFiltres = computed(() => {
    ...
    return movs.filter(m => {
      ...
      const correspondLot = this.lotFiltre() === '' || m.lot.toLowerCase().includes(this.lotFiltre().toLowerCase());
      const correspondFournisseur = this.fournisseurFiltre() === '' || m.fournisseurId === this.fournisseurFiltre();
      
      return correspondRecherche && correspondType && correspondDate && correspondLot && correspondFournisseur;
  ...
  ouvrirModale(direction: 'entree' | 'sortie') {
    ...
    setTimeout(() => {
      ...
      this.fournisseurSelectionneId.set(this.serviceDonnees.fournisseurs()[0]?.id || '');
  ...
  soumettreMouvement(evenement: Event) {
    ...
    const observablesLots = this.lignesMouvement().map((ligne, index) => {
      ...
      if (lotExistant) {
        ...
        // Récupération du fournisseur sélectionné ou par défaut
        const validFournisseurId = this.fournisseurSelectionneId() || 
                                   this.serviceDonnees.fournisseurs()[0]?.id || 
                                   this.serviceDonnees.lots()[0]?.fournisseurId;

        if (!validFournisseurId) {
          return of({ index, error: `Impossible de créer le lot "${nomNumLot}" : aucun fournisseur n'est sélectionné ou enregistré.` });
        }
