...
class PrevisionSerializer(serializers.ModelSerializer):
    medicamentId = serializers.PrimaryKeyRelatedField(source='medicament', queryset=Medicament.objects.all())
    etablissementId = serializers.PrimaryKeyRelatedField(source='etablissement', queryset=Etablissement.objects.all(), required=False, allow_null=True)
    regionId = serializers.PrimaryKeyRelatedField(source='region', queryset=Region.objects.all(), required=False, allow_null=True)
    etablissementSourceTransfertId = serializers.PrimaryKeyRelatedField(source='etablissementSourceTransfert', queryset=Etablissement.objects.all(), required=False, allow_null=True)
    # Champs en lecture seule pour l'affichage
    medicamentNom = serializers.CharField(source='medicament.dci', read_only=True)
    etablissementNom = serializers.CharField(source='etablissement.nom', read_only=True, default=None)
    regionNom = serializers.CharField(source='region.nom', read_only=True, default=None)

    class Meta:
        model = Prevision
        fields = (
            'id', 'medicamentId', 'medicamentNom',
            'etablissementId', 'etablissementNom',
            'regionId', 'regionNom',
            'niveau', 'mois', 'dateFocalisation',
            'demandePrevue', 'confiance', 'facteurs',
            'stockActuelCalcule', 'risqueRupture', 'dateRuptureEstimee',
            'recommandationCommande', 'quantiteCommandeRecommandee',
            'recommandationTransfert', 'etablissementSourceTransfertId',
        )
