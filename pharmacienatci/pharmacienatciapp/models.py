...
class Prevision(models.Model):
    NIVEAU_CHOICES = [('etablissement', 'Établissement'), ('region', 'Région')]
    id = models.CharField(max_length=50, primary_key=True)
    medicament = models.ForeignKey(Medicament, on_delete=models.CASCADE, related_name='previsions')
    mois = models.CharField(max_length=10)  # Format: YYYY-MM
    # Niveau d'analyse
    niveau = models.CharField(max_length=20, choices=NIVEAU_CHOICES, default='etablissement')
    etablissement = models.ForeignKey(Etablissement, null=True, blank=True, on_delete=models.CASCADE, related_name='previsions')
    region = models.ForeignKey(Region, null=True, blank=True, on_delete=models.CASCADE, related_name='previsions')
    # Métriques de prévision
    demandePrevue = models.DecimalField(max_digits=15, decimal_places=2)
    confiance = models.DecimalField(max_digits=5, decimal_places=2, default=0)  # 0-100
    facteurs = models.JSONField(default=list, blank=True)
    # Métriques de stock et rupture
    stockActuelCalcule = models.DecimalField(max_digits=15, decimal_places=2, default=0)
    risqueRupture = models.BooleanField(default=False)
    dateRuptureEstimee = models.DateField(null=True, blank=True)
    # Recommandations IA
    recommandationCommande = models.BooleanField(default=False)
    quantiteCommandeRecommandee = models.DecimalField(max_digits=15, decimal_places=2, default=0)
    recommandationTransfert = models.BooleanField(default=False)
    etablissementSourceTransfert = models.ForeignKey(Etablissement, null=True, blank=True, on_delete=models.SET_NULL, related_name='previsions_source_transfert')
    dateFocalisation = models.DateField(auto_now_add=True)

    def __str__(self):
        return f"Prévision {self.medicament.dci} - {self.mois}"
